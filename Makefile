...
# funa
        ...
        $(info >>> Minimum GCC version required is 5.1.0<<< but run make anyway)
...
ALL_LDFLAGS += -lm $(CUBLAS_LIBS)
OUTPUT_TXT := vgg19_output_1000.txt
INPUT_TXT := vol.txt
PRETRAINED_MODEL := imagenet-vgg-verydeep-19.mat
WEIGHT_TXT := vgg19_weight.txt
BIAS_TXT := vgg19_bias.txt
...
clean:
	rm -f $(PROG) $(OBJS) $(OUTPUT_TXT)

test: $(PROG)
	$(EXEC) ./$(PROG)

$(PRETRAINED_MODEL):
	curl -O https://www.vlfeat.org/matconvnet/models/$(PRETRAINED_MODEL)

$(WEIGHT_TXT): $(PRETRAINED_MODEL)
	python vgg19.py

$(BIAS_TXT): $(PRETRAINED_MODEL)
	python vgg19.py
